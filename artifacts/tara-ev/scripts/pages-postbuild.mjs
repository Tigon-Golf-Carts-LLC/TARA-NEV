 ...
 *   5. insecure    — fails the build on any http:// subresource, which would
 *                    otherwise cost the page its HTTPS padlock.
 *   6. image prune — drops images no page references. The source tree carries
...
// Same policy the built pages carry, minus the directives a redirect stub has
// no use for. No inline <script> here: a meta refresh needs no JS, so the
// stub stays compliant with script-src 'self' without a hash exemption.
const REDIRECT_CSP =
  "default-src 'none'; form-action 'none'; base-uri 'self'; " +
...
if (!fs.existsSync(shellPath)) {
  // The shell is consumed below, so a re-run against an already-processed
  // dist finds it gone. That is fine as long as 404.html is already there;
  // a missing shell AND no 404.html means prerender.mjs never ran.
  if (!fs.existsSync(notFoundPath)) {
    console.error(
      `[pages] ERROR: neither "${shellPath}" nor "${notFoundPath}" exists — ` +
        "prerender.mjs must run first.",
    );
    process.exit(1);
  }
  console.log("[pages] 404.html already present; leaving it as-is.");
}
...
if (fs.existsSync(shellPath)) {
  ...
  notFound = notFound
    .replace(
      /<title>[^<]*<\/title>/,
      "<title>Page not found — TARA NEV</title>",
    )
    .replace(
...
// ─── Insecure-content guard ───────────────────────────────────────────────────
//
// A single http:// subresource is enough to cost the whole page its padlock.
// The CSP's upgrade-insecure-requests directive rewrites them at fetch time,
// but that is a safety net, not a licence to ship them — an upgraded URL still
// 404s if the remote host has no TLS. Fail the build instead.
//
// Only fetched subresources count. http:// inside XML namespaces
// (www.w3.org), JSON-LD @context values (schema.org), sitemap schema URLs and
// vendored-library licence comments are identifiers, never requests.
...
const insecureRefs = [];
for (const [rel, abs] of walkFiles(outDir).map(([r, a]) => [r, a])) {
  if (!/\.(html|css)$/.test(abs)) continue;
  const text = fs.readFileSync(abs, "utf8");
  // Attribute-based subresource loads plus CSS url() references.
  const patterns = [
    /(?:src|href|srcset|poster|data-src)\s*=\s*["'](http:\/\/[^"']+)["']/gi,
    /url\(\s*["']?(http:\/\/[^)"']+)/gi,
  ];
  for (const pattern of patterns) {
    for (const match of text.matchAll(pattern)) {
      const url = match[1];
      // A plain <a href="http://..."> is a link the user clicks, not a
      // subresource — it cannot break the padlock.
      if (pattern.source.includes("src") && /href/i.test(match[0])) {
        const tagStart = text.lastIndexOf("<", match.index);
        if (/^<a[\s>]/i.test(text.slice(tagStart, tagStart + 3))) continue;
      }
      if (IDENTIFIER_HOSTS.some((h) => url.startsWith(h))) continue;
      insecureRefs.push(`${rel}: ${url}`);
    }
  }
}

if (insecureRefs.length) {
  console.error(
    `[pages] ERROR: ${insecureRefs.length} insecure http:// subresource(s) in ` +
      "the build output. These break HTTPS for the whole page — switch them to " +
      "https:// or localize the asset:",
  );
  for (const ref of insecureRefs.slice(0, 20)) console.error(`  ${ref}`);
  if (insecureRefs.length > 20) {
    console.error(`  …and ${insecureRefs.length - 20} more.`);
  }
  process.exit(1);
}
console.log("[pages] No insecure http:// subresources.");
...
console.log(
  `[pages] Build output: ${totalMb} MB (limit 1024 MB). Ready to deploy.`,
);
